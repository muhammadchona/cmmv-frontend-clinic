    ...
    <template v-slot:body="props">
      <q-tr :props="props">
        ...
        <q-td key="hasHappened" :props="props">
          <q-checkbox
            v-model="props.row.hasHappened"
            color="primary"
            v-if="
              moment(new Date(), 'DD-MM-YYYY') <
              moment(visitDatePlusTwoDays(props.row.visitDate, 'DD-MM-YYYY'))
            "
          ...
          <q-checkbox
            ...
            @click="getSelectedString(props.row)"
          />
          <span v-if="props.row.hasHappened">
            Sim
            <q-icon
              name="info"
              ...
              v-if="
                ...
                new Date(
                  visitDatePlusTwoDays(props.row.visitDate).setHours(0, 0, 0, 0)
                ).getTime()
              "
            >
              <q-tooltip
              ...
              >
                <strong
                  ...
                  {{
                    formatDateDDMMMYYYY(
                      visitDatePlusTwoDays(props.row.visitDate)
                    )
                  }}</strong
                >
                não poderá <em>editar esta consulta</em>
              </q-tooltip>
            </q-icon>
          </span>
        </q-td>
...
import AppointmentTableXls from 'src/utils/AppointmentTableXls';
...
const diffBlockDays = (visitPlus2, today) => {
  ...
  return date.formatDate(
    moment(visitPlus2).subtract(diff, 'days'),
    'DD-MM-YYYY'
  );
};
...
const visitDatePlusTwoDays = (visitDate) => {
  const result = addToDate(
    moment(visitDate).set('hour', '23').set('minute', '59').set('second', '59'),
    { days: 2 }
  );
  return result;
};
...
const exportTable = () => {
  if (!isMobile.value) {
    ...
    AppointmentTableXls.downloadExcel(props.rows);
  }
};
